import { escapeMd, escapeXml } from "./escapes";

const toMd = (node: any): any => {
  if (node === undefined) {
    return "";
  }
  ...
  switch (node.type) {
    ...
    case "simple_link":
      return (
        node.args.url +
        (node.args.text && node.args.text !== node.args.url
          ? " [" + escapeMd(node.args.text) + "] "
          : "")
      );
    case "image":
      return (
        node.args.url +
        (node.args.alt && node.args.alt !== node.args.url
          ? " [" + escapeMd(node.args.alt) + "] "
          : "") +
        "\n"
      );
    case "error":
    ...
    case "quote":
      return (
        (node.args.cite ? "(" + escapeMd(node.args.cite) + ")\n" : "") +
        "> " +
    ...
    case "ruby":
      return (
        "{#ruby=" + escapeMd(node.args.text) + " " + toMd(node.content) + "}"
      );
    case "spoiler":
      return (
        (node.args.label
          ? "(spoiler: " + escapeMd(node.args.label) + ") "
          : "") +
        "||" +
        toMd(node.content) +
        "||\n"
    ...
    case "background_color":
      return (
        "{#bg=" + escapeMd(node.args.color) + " " + toMd(node.content) + "}"
      );
    default:
      return toMd(node.content);
  }
...
let autocloseTags: AutoCloseTags = {
  br: true,
  img: true,
  hr: true,
};
...
const renderNode = (node: any) => {
  ...
  if (node.tag) {
    ...
    if (autocloseTags[node.tag as string]) {
      out += " />";
      return out;
    }
...
const markuprenderToHtml = function ({ args, content }: any) {
  try {
...
const markuprenderToMd = function ({ args, content }: any) {
  try {
    return toMd(content);
  } catch (err) {
