        ...
        const altIsSame =
          ...
          (token.content &&
            ...
            (token.content[0].content === "" ||
              token.content[0].content === token.target));
        out +=
          token.target + (altIsSame ? "" : "[" + to12y(token.content) + "]");
        break;
      }
      case "blockQuote":
        out += ">{" + to12y(token.content) + "}\n";
...
export const mdto12y = (text: string) => {
